...
public abstract class ComponentBase : Microsoft.AspNetCore.Components.ComponentBase, IDisposable, IAsyncDisposable
{
      ...
      Expression<Func<TViewModel, TValue>> property)
  {
    if (viewModel is null)
      throw new Langueedu.Components.Internal.Bindings.BindingException("ViewModelType is null");

    if (property is null)
      throw new Langueedu.Components.Internal.Bindings.BindingException("Property expression is null");

    if (!(property.Body is MemberExpression m))
      throw new Langueedu.Components.Internal.Bindings.BindingException("Binding member needs to be a property");

    if (!(m.Member is PropertyInfo p))
      throw new Langueedu.Components.Internal.Bindings.BindingException("Binding member needs to be a property");

    if (typeof(TViewModel).GetProperty(p.Name) is null)
      throw new Langueedu.Components.Internal.Bindings.BindingException($"Cannot find property {p.Name} in type {viewModel.GetType().FullName}");
